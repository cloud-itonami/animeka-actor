(ns animekaza.aozora
  "Real app-aozora Publisher for animeka — creates a record in the
  com.etzhayyim.apps.animeka.clip collection on an aozora PDS via the
  ...
  tashikame.aozora (keep in sync — same createSession(self-CACAO)→JWT→
  createRecord flow that app-aozora-pds enforces, ADR-2606251700 /
  DEPLOY-RUNBOOK).

  When produced media exists, the clip record carries the
...
(defn aozora-publisher
  "Returns an `animekaza.publisher/Publisher` that creates clip records on
  the aozora PDS. opts:
