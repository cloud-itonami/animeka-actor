(ns animekaza.cacao
  "Agent-side CACAO issuance (JVM). The animeka actor mints its OWN
  ...
  Per-actor key model: animeka generates + persists its OWN Ed25519 key
  ...
  The private key is persisted to `.animeka/identity.edn` (gitignored) — NEVER
...
(defn canonical-graph
  "The deterministic graph CID for one of animeka's databases. The edge
...
(def default-db-name
  "animeka's primary database — the clip ledger."
  "animeka")
...
(defn load-or-create-identity!
  "Per-actor key: load animeka's persisted Ed25519 identity at `path`, or
