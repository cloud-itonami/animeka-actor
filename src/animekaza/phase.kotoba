(ns animekaza.phase
  "Phase 0→2 staged rollout for animeka (ADR-2607071300 shape, publish policy
  as revised by superproject ADR-2607162200 Layer D — adopted from R0 here,
  no per-clip-human-only interregnum). The phase gate can only ever withhold
  ...
  in any phase.
    ...
    Phase 2  public     — public announcement with an explicit approval in
                          ...
                          :publish       per-clip human sign-off
                          ...
                          :auto-publish  the scheduled outer loop's standing
                                         ...
                                         judgment; the AnimekaGovernor's HARD
...
(defn publish-allowed?
  "May a governor-clean clip be announced under `phase` with the run's
